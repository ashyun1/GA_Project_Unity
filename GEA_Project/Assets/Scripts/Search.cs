...
public class Search : MonoBehaviour
{
    ...
    private int Partition(List<Item> list, int left, int right) // 쉼표 추가
    {
        ...
        {
            sortSteps++;
            if (list[j].itemName.CompareTo(pivot.itemName) <= 0) // 오타 수정
            {
                i++;
                Swap(list, i, j);
            }
    ...
    {
        int steps = 0;
        foreach (Item item in items) // 오타 수정
        {
            steps++;
            if (item.itemName == target) // 변수 접근 수정
                return steps;
        }
        return steps;
    }
